 ...
 * @time 2:35 PM
 */
public class GetDeviceCapacityApi extends AbstractAPI{

    private String deviceSerial;
    private HttpPostMethod httpPostMethod;
    public GetDeviceCapacityApi(String accessToken, String deviceSerial) {
        this.url = ServerConstant.GET_DEVICE_Capacity;
    ...
    public BasicResponse<DeviceCapacityVO> executeApi() {
        BasicResponse<DeviceCapacityVO> response = null;
        HttpResponse httpResponse = httpPostMethod.execute();

        try {
            response = JSON.parseObject(httpResponse.getEntity().getContent(), BasicResponse.class);
//            response.setJson(JSON.toJSONString(response));
//            Object data = JSON.parseObject(JSON.toJSONString(response.getDataInternal()), DeviceCapacityVO.class);
